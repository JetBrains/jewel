...
public val ToolwindowGroup.`Changes@20x20`: IntellijIconData
    get() {
        if (`_changes@20x20` != null) {
            return `_changes@20x20`!!
        }
        `_changes@20x20` = IntellijIconData(imageVector = { `Changes@20x20Composable`(it) })
        return `_changes@20x20`!!
    }

private var `_changes@20x20`: IntellijIconData? = null

private fun `Changes@20x20Composable`(colorScheme: IntelliJIconColors): ImageVector = Builder(name =
        "Changes@20x20", defaultWidth = 20.0.dp, defaultHeight = 20.0.dp, viewportWidth = 20.0f,
