...
public val VcsGroup.`Update@20x20`: IntellijIconData
    get() {
        if (`_update@20x20` != null) {
            return `_update@20x20`!!
        }
        `_update@20x20` = IntellijIconData(imageVector = { `Update@20x20Composable`(it) })
        return `_update@20x20`!!
    }

private var `_update@20x20`: IntellijIconData? = null

private fun `Update@20x20Composable`(colorScheme: IntelliJIconColors): ImageVector = Builder(name =
        "Update@20x20", defaultWidth = 20.0.dp, defaultHeight = 20.0.dp, viewportWidth = 20.0f,
