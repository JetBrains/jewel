...
public val JavaeeGroup.WebService: IntellijIconData
    get() {
        if (_webService != null) {
            return _webService!!
        }
        _webService = IntellijIconData(name = "WebService",size = DpSize(height = 16.0.dp, width =
                16.0.dp),imageVector = { WebServiceComposable(it) })
        return _webService!!
    }

private var _webService: IntellijIconData? = null

private fun WebServiceComposable(colorScheme: IntelliJIconColors): ImageVector = Builder(name =
        "WebService", defaultWidth = 16.0.dp, defaultHeight = 16.0.dp, viewportWidth = 16.0f,
