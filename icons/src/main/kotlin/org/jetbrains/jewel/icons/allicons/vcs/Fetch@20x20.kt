...
public val VcsGroup.`Fetch@20x20`: IntellijIconData
    get() {
        if (`_fetch@20x20` != null) {
            return `_fetch@20x20`!!
        }
        `_fetch@20x20` = IntellijIconData(imageVector = { `Fetch@20x20Composable`(it) })
        return `_fetch@20x20`!!
    }

private var `_fetch@20x20`: IntellijIconData? = null

private fun `Fetch@20x20Composable`(colorScheme: IntelliJIconColors): ImageVector = Builder(name =
        "Fetch@20x20", defaultWidth = 20.0.dp, defaultHeight = 20.0.dp, viewportWidth = 20.0f,
