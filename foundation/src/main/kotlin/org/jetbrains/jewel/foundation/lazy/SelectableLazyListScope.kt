...
public interface SelectableLazyListScope {
     ...
     *   Defaults to `true`.
     * @param itemContent The content of each individual item, specified as a composable function
     *   that takes the item's index as a parameter.
     */
    public fun items(
        count: Int,
        key: (index: Int) -> Any,
...
internal class SelectableLazyListScopeContainer : SelectableLazyListScope {
    /**
     * Provides a set of keys that cannot be selected. Here we use an assumption that amount of
     * selectable items >> amount of non-selectable items. So, for optimization we will keep only
     * this set.
     *
     * @see [isKeySelectable]
     */
