...
internal abstract class LazyTableScrollbarAdapter : ScrollbarAdapter {
    ...
    private suspend fun snapTo(scrollOffset: Double) {
        ...
        val offset =
            (scrollOffsetCoerced - index * averageVisibleLineSizeWithSpacing)
                .toInt()
                .coerceAtLeast(0)
...
internal class LazyTableHorizontalScrollbarAdapter(
    ...
    override val viewportSize: Double
        get() =
            with(scrollState.layoutInfo) {
                viewportSize.width - pinnedColumnsWidth
            }.toDouble()

    override fun firstVisibleLine(): VisibleLine? {
        val item =
            scrollState.layoutInfo.floatingItemsInfo.firstOrNull()
                ?: return null
        return VisibleLine(
    ...
    override fun totalLineCount() = if (scrollState.layoutInfo.rows > 0) scrollState.layoutInfo.columns - scrollState.layoutInfo.pinnedColumns else 0
    ...
    override fun averageVisibleLineSize(): Double {
        val first =
            scrollState.layoutInfo.floatingItemsInfo.firstOrNull()
                ?: return 0.0
        val last =
            scrollState.layoutInfo.floatingItemsInfo.lastOrNull()
                ?: return 0.0
...
internal class LazyTableVerticalScrollbarAdapter(
    ...
    override val viewportSize: Double
        get() =
            with(scrollState.layoutInfo) {
                viewportSize.height - pinnedRowsHeight
            }.toDouble()
    ...
    override fun totalLineCount() = if (scrollState.layoutInfo.columns > 0) scrollState.layoutInfo.rows - scrollState.layoutInfo.pinnedRows else 0
...
public fun tableHorizontalScrollbarAdapter(scrollState: LazyTableState): ScrollbarAdapter = LazyTableHorizontalScrollbarAdapter(scrollState)
...
public fun tableVerticalScrollbarAdapter(scrollState: LazyTableState): ScrollbarAdapter = LazyTableVerticalScrollbarAdapter(scrollState)
