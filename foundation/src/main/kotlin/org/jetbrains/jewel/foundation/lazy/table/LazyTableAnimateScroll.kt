...
private class LineFoundInScroll(
    val lineOffset: Int,
    val previousAnimation: AnimationState<Float, AnimationVector1D>,
) : CancellationException()

internal suspend fun LazyTableAnimateScrollScope.animateScrollToItem(
    index: Int,
    scrollOffset: Int,
) {
    scroll {
            ...
            fun isOvershot(): Boolean {
                ...
                return if (forward) {
                    if (firstVisibleLineIndex > index) {
                        ...
                        firstVisibleLineIndex == index &&
                        firstVisibleLineScrollOffset > scrollOffset
                    ) {
                        true
                    } else {
                        false
...
//                            debugLog { "Found the item after performing scrollBy()" }
                            ...
                            if (forward) {
                                if (
                                    loops >= 2 &&
                                    index - lastVisibleLineIndex > numOfLinesForTeleport
                                ) {
                                    // Teleport
                                    myLogger().debug("Teleport forward")
                                    snapToLine(
                                        index = index - numOfLinesForTeleport,
                                        scrollOffset = 0,
                                    )
                                }
                            } else {
                                if (
                                    loops >= 2 &&
                                    firstVisibleLineIndex - index > numOfLinesForTeleport
                                ) {
                                    // Teleport
                ...
                val coercedValue =
                    when {
                        ...
                        target < 0 -> {
                            ...
                            myLogger().debug("WARNING: somehow ended up seeking 0px, this shouldn't happen")
                            0f
                        }
                    }
                val delta = coercedValue - prevValue
...
internal class LazyTableAnimateHorizontalScrollScope(
    private val state: LazyTableState,
) : LazyTableAnimateScrollScope {
    override val density: Density get() = state.density

    override val firstVisibleLineIndex: Int get() = state.firstVisibleColumnIndex

    override val firstVisibleLineScrollOffset: Int get() = state.firstVisibleItemHorizontalScrollOffset

    override val lastVisibleLineIndex: Int
        get() =
            state.layoutInfo.pinnedRowsInfo
                .lastOrNull()
                ?.column
                ?: state.layoutInfo.floatingItemsInfo
                    .lastOrNull()
                    ?.column
                ?: 0
    ...
    override fun ScrollScope.snapToLine(
        index: Int,
        scrollOffset: Int,
    ) {
    ...
    override fun expectedDistanceTo(
        ...
        val averageSize =
            visibleItems.fastSumBy { it.size.width } / visibleItems.size + layoutInfo.horizontalSpacing
        ...
        return (averageSize * indexesDiff).toFloat() +
            coercedOffset - firstVisibleLineScrollOffset
    }
...
internal class LazyTableAnimateVerticalScrollScope(
    private val state: LazyTableState,
) : LazyTableAnimateScrollScope {
    override val density: Density get() = state.density

    override val firstVisibleLineIndex: Int get() = state.firstVisibleRowIndex

    override val firstVisibleLineScrollOffset: Int get() = state.firstVisibleItemVerticalScrollOffset

    override val lastVisibleLineIndex: Int
        get() =
            state.layoutInfo.pinnedColumnsInfo
                .lastOrNull()
                ?.row
                ?: state.layoutInfo.floatingItemsInfo
                    .lastOrNull()
    ...
    override fun expectedDistanceTo(
        ...
        val visibleItems = layoutInfo.floatingItemsInfo
        val averageSize =
            visibleItems.fastSumBy { it.size.height } / visibleItems.size + layoutInfo.verticalSpacing
        val indexesDiff = index - firstVisibleLineIndex
        var coercedOffset = minOf(abs(targetScrollOffset), averageSize)
        if (targetScrollOffset < 0) coercedOffset *= -1
        return (averageSize * indexesDiff).toFloat() +
