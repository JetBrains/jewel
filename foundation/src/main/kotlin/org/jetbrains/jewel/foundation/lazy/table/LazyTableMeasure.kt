...
internal fun measureLazyTable(
    ...
    if (rows * columns <= 0) {
        ...
        val maxCellAxisX = maxOffsetX
        val maxCellAxisY = maxOffsetY
        ...
        do {
            val needNextColumn = currentLastFloatingColumn < columns &&
                (currentCellAxisOffsetX < maxCellAxisX || currentCellAxisOffsetX <= 0)
            val needNextRow = currentLastFloatingRow < rows &&
                (currentCellAxisOffsetY < maxCellAxisY || currentCellAxisOffsetY <= 0)
        ...
        if (currentCellAxisOffsetX < maxCellAxisX) {
            val toScrollBack = maxCellAxisX - currentCellAxisOffsetX
        ...
        if (currentCellAxisOffsetY < maxCellAxisY) {
            val toScrollBack = maxCellAxisY - currentCellAxisOffsetY
        ...
        val consumedScrollX = if (scrollToBeConsumed.x.roundToInt().sign == scrollDeltaX.sign &&
        ...
        val consumedScrollY = if (scrollToBeConsumed.y.roundToInt().sign == scrollDeltaY.sign &&
        ...
        val visibleRows = currentFirstFloatingRow until currentLastFloatingRow
        val visibleColumns = currentFirstFloatingColumn until currentLastFloatingColumn
        ...
        repeat(pinnedRows) { row ->
            repeat(pinnedColumns) { column ->
                pinnedItemsInfo += measuredItemProvider.getAndMeasure(column, row)
            }
        }
        ...
        return LazyTableMeasureResult(
            ...
            measureResult = layout(constraints.maxWidth, constraints.maxHeight) {
                floatingItemsInfo.fastForEach {
                    it.place(this)
                }

                pinnedColumnsInfo.fastForEach {
                    it.place(this, 1f)
                }

                pinnedRowsInfo.fastForEach {
                    it.place(this, 1f)
                }

                pinnedItemsInfo.fastForEach {
                    it.place(this, 1f)
                }
            },
...
private fun calculateItemsOffsets(
    ...
    fun getCellScrollOffsetX(column: Int): Int = positionedColumns.getOrPut(column) {
    ...
    fun getCellScrollOffsetY(row: Int): Int = positionedRows.getOrPut(row) {
    ...
    fun getPinnedCellScrollOffsetX(column: Int): Int = positionedPinnedColumns.getOrPut(column) {
        if (column > 0) {
            val previousColumn = column - 1
            val previousColumnWidth = measuredItemProvider.getColumnWidth(previousColumn)
    ...
    fun getPinnedCellScrollOffsetY(row: Int): Int = positionedPinnedRows.getOrPut(row) {
        if (row > 0) {
            val previousRow = row - 1
            val previousRowHeight = measuredItemProvider.getRowHeight(previousRow)
    ...
    fun getScrollOffsetX(column: Int): Int = if (column < pinnedColumns) {
        getPinnedCellScrollOffsetX(column)
    } else {
        getCellScrollOffsetX(column)
    }

    fun getScrollOffsetY(row: Int): Int = if (row < pinnedRows) {
