 ...
 *
 * @param extensions Extensions to use when processing the Markdown (e.g., to support parsing custom
 *   block-level Markdown).
 * @param editorMode Indicates whether the processor should be optimized for an editor/preview
 *   scenario, where it assumes small incremental changes as performed by a user typing. This means
 *   it will only update the changed blocks by keeping state in memory.
 *
 *   Default is `false`; set this to `true` if this parser will be used in an editor scenario, where
 *   the raw Markdown is only ever going to change slightly but frequently (e.g., as the user
 *   types).
 *
 *   **Attention:** do **not** reuse or share an instance of [MarkdownProcessor] that is in
 *   [editorMode]. Processing entirely different Markdown strings will defeat the purpose of the
 *   optimization. When in editor mode, the instance of [MarkdownProcessor] is **not** thread-safe!
 *
 * @param commonMarkParser The CommonMark [Parser] used to parse the Markdown. By default it's a
 *   vanilla instance provided by the [MarkdownParserFactory], but you can provide your own if you
 *   need to customize the parser — e.g., to ignore certain tags. If [optimizeEdits] is `true`, make
 *   sure you set `includeSourceSpans(IncludeSourceSpans.BLOCKS)` on the parser.
 */
...
public class MarkdownProcessor(
    ...
    /**
     * Parses a Markdown document, translating from CommonMark 0.31.2 to a list of [MarkdownBlock].
     * Inline Markdown in leaf nodes is contained in [InlineMarkdown], which can be rendered to an
     * [androidx.compose.ui.text.AnnotatedString] by using
        ...
        outerLoop@ for ((i, spans) in previousIndexes.withIndex().reversed()) {
            ...
            for (j in begin until currLastLine) {
                ...
                if (newIndex < 0 ||
                    ...
                    previousLines[j] != newLines[newIndex]) {
                    break@outerLoop
                }
            }
    ...
    /**
     * Processes the children of a CommonMark [Node]. This function is public so that it can be
     * accessed from [MarkdownProcessorExtension]s, but should not be used in other scenarios.
     */
    @InternalJewelApi
