...
open class StudioVersionsGenerationExtension(project: Project) {
  ...
  val resourcesDirs: SetProperty<File> =
    project.objects.setProperty<File>().convention(project.sourceSets.main.get().resources.srcDirs)
...
open class AndroidStudioReleasesGeneratorTask : DefaultTask() {
  ...
  fun generate() {
    ...
    val releases = URL(url).openStream().use { json.decodeFromStream<ApiAndroidStudioReleases>(it) }
