 ...
 * Only available when running in the IntelliJ Platform.
 *
 * **Note:** when using the Classic UI on Windows, this returns the same value as [regular] (the
 * default TextStyle from the theme). This is the same behavior implemented by
...
/**
 * The text style to use for small text. Smaller than [medium]. Should be avoided when running in
 * the New UI, in favor of [medium], unless it's absolutely necessary.
 *
 * Only available when running in the IntelliJ Platform.
 *
 * **Note:** when using the Classic UI on Windows, this returns the same value as [regular] (the
