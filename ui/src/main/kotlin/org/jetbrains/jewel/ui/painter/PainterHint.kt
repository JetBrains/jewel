 ...
 * [Painter][androidx.compose.ui.graphics.painter.Painter]. It can used to patch the path of the
 * resource being loaded (e.g., for New UI icon path mapping, and handling the dark theme variants),
 * replace colors in an SVG based on the theme palette, etc.
 *
...
/**
 * A [PainterHint] that modifies the path of the resource being loaded. Usage examples are applying
 * the New UI icon mappings, or picking up dark theme variants of icons.
 */
@Immutable
...
/**
 * A [PainterHint] that patches the content of SVG resources. It is only applied to SVG resources;
 * it doesn't affect other types of resources.
 */
@Immutable
...
/**
 * A [PainterHint] that adds a prefix to a resource file name, without changing the rest of the
 * path. For example, if the original path is `icons/MyIcon.svg`, and the prefix is `Dark`, the
 * patched path will be `icons/DarkMyIcon.svg`.
 */
@Immutable
...
/**
 * A [PainterHint] that adds a suffix to a resource file name, without changing the rest of the path
 * nor the extension. For example, if the original path is `icons/MyIcon.svg`, and the suffix is
 * `_dark`, the patched path will be `icons/MyIcon_dark.svg`.
 */
@Immutable
