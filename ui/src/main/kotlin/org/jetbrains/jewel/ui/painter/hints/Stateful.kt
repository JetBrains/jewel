...
/**
 * Selects a stateful variant of an image, based on the current [state]. Stateful variants are
 * `Focused`, `Pressed`, `Hovered`, and `Disabled`. If an image does not have the required stateful
 * variant, the base one will be used.
 *
 * If the [state] is a [FocusableComponentState] and its
 * [`isFocused`][FocusableComponentState.isFocused] property is true, then the `Focused` variant
 * will be used.
 *
 ...
 *
 * Note that the
 * [Swing Compat mode][org.jetbrains.jewel.foundation.theme.JewelTheme.isSwingCompatMode] value
 * might prevent the selection of the pressed and hovered states, when true.
 */
