...
public object SpinnerProgressIconGenerator {
    ...
    private fun StringBuilder.openTag(sizePx: Int) =
        append(
    ...
    private fun StringBuilder.drawElement(
        ...
        append(
            "<rect fill=\"${colorHex}\" opacity=\"$opacity\" x=\"$x\" y=\"$y\" width=\"$width\" height=\"$height\" rx=\"$rx\"",
        )
    ...
    public object Small {
        ...
        public fun generateRawSvg(colorHex: String): List<String> =
            getPlainTextSvgList(colorHex = colorHex, size = 16)
    }
